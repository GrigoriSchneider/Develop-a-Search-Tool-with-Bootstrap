...
},{"@popperjs/core":"../node_modules/@popperjs/core/lib/index.js"}],"recipes.js":[function(require,module,exports) {
"use strict";
...
var containerTags = document.getElementById('container-tags');
var recipes = _recipes.recipesjson.recipes; // tagsarray 

var tagsArray = []; // recepesId

var recipesIdArray = []; // Ingredients

var ingredients = [];
var showIngredients = []; // Device

var devices = [];
var showDevice = []; // Ustensils

var ustensils = [];
var showUstensils = []; // showReceps

var showRecepsArray = []; // Filter Inputs
...
var inputUstensils = document.getElementById('input-ustensils'); // InnerHtml for cards

function recipesCardsHtml() {
  var html = '';

  for (var i = 0; i < recipes.length; i++) {
    var recipesId = recipes[i].id;
    var recipesName = recipes[i].name;
    var recipesDescription = recipes[i].description;
    var recipesTime = recipes[i].time;
    var ustensil = recipes[i].ustensils;
    ...
    for (var j = 0; j < recipes[i].ingredients.length; j++) {
      var ingredientName = recipes[i].ingredients[j].ingredient;
      var ingredientQuantity = recipes[i].ingredients[j].quantity;
      var ingredientUnit = recipes[i].ingredients[j].unit;
      var device = recipes[i].appliance;
      ...
      html += ""; // Ingredients array

      ingredients.push(ingredientName); // devices array
...
function filterArrayUnique(array, newArray) {
  ...
  for (var _i = 0; _i < array.length; _i++) {
    arrayLowerCase.push(array[_i].toLowerCase());
  } // Filter all duplicates


  var uniqueSet = new Set(arrayLowerCase); // back to array

  var noDuplicates = _toConsumableArray(uniqueSet); //capitalize only the first letter of the array
...
function recipesDropdown() {
  ...
  dropdownIngredients.innerHTML = html; // reset Arrays

  ingredients = [];
  showIngredients = [];
}

recipesDropdown(); // deviceDropdown
...
inputIngredients.addEventListener('keyup', function filterFunction(e) {
  var keyword = inputIngredients.value.toLowerCase();
  var filteredArray = showIngredients.filter(function (ingredient) {
...
window.replyForFilterTags = function (addtoArray) {
  ...
  if (tagsArray.includes(addtoArray) == false) {
    tagsArray.push(addtoArray); // Filter the Recipes

    filterRecipes(); // console.log(tagsArray);
    // Add tags to html
...
function filterRecipes() {
  var ingredientsFromID = [];
  var counterHideCards = 1;

  for (var i = 0; i < recipes.length; i++) {
    var recipesId = recipes[i].id;
    var allIngredients = recipes[i].ingredients; // all card get class hide
    ...
    for (var j = 0; j < allIngredients.length; j++) {
      for (var k = 0; k < tagsArray.length; k++) {
        ...
        if (tagsArray[k].toLowerCase() === allIngredients[j].ingredient.toLowerCase()) {
          ...
          ingredientsFromID.push(recipesId); // Remove class hide
...
function createForLiArray(arrayIds) {
  ...
  for (var i = 0; i < arrayIds.length; i++) {
    var ingredient = recipes[arrayIds[i]].ingredients;
    ...
    for (var j = 0; j < ingredient.length; j++) {
      ...
      if (ingredient[j].ingredient === undefined) {
        ...
        ingredients.push(ingredient[j].ingredient); // console.log(ingredient[j].ingredient);
      }
  ...
  recipesDropdown(); // ingredient = [];
}

;
},{"bootstrap":"../node_modules/bootstrap/dist/js/bootstrap.esm.js","./recipes.js":"recipes.js"}],"../../../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
